...
export default function ConsultationBookingPage() {
  ...
  const handleBooking = () => {
    if (!bookerName || !date || !time || (role === 'parent' && !studentName)) {
      toast({
        title: 'خطأ',
    ...
    try {
      ...
      router.push(role === 'parent' ? '/parent-space' : '/teacher-space');
    } catch (error) {
  ...
  return (
    <div className="flex items-center justify-center h-full animate-in fade-in-50">
      <Card className="w-full max-w-2xl bg-card/50 rounded-2xl p-8">
        <CardHeader>
          <CardTitle className="font-headline text-2xl text-center">حجز استشارة نفسية (عن بعد)</CardTitle>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="space-y-2">
            <Label htmlFor="booker-name">اسمك الكامل</Label>
            <Input
              id="booker-name"
              value={bookerName}
